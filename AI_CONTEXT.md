...
| Forms | `/contact` POSTs to `/api/contact` (persists to `ContactSubmission`). `/demo` POSTs to `/api/demo-request` (persists to `DemoRequest`). Both rate-limited (5/min/IP), zod-validated, **honeypot-protected** (hidden `website` field), and store **only an `ipHash`** (SHA-256 of `IP + JWT_SECRET`) — no raw IP. Field names match spec (`workEmail`, `aiToolsUsed`, `preferredTime`, `companySize`). Success states: "Message received. Our team will review it and respond soon." / "Demo request received. Our team will contact you to schedule a time." Mailto fallback still offered as a secondary path on every form. |
